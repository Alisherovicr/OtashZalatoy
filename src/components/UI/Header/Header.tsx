...
import "./Header.scss"; // Добавлен импорт стилей
import { searchSongs } from "../../../store/API/searchApi";

interface HeaderProps {
  handleToggleFavorite: (result: any) => void; // Пропс для функции добавления в избранное
}

export const Header: React.FC<HeaderProps> = ({ handleToggleFavorite }) => {
  const [isMenuOpen, setIsMenuOpen] = useState<boolean>(false);
  const [searchResults, setSearchResults] = useState<any[]>([]); // Состояние для результатов поиска
  const [searchQuery, setSearchQuery] = useState<string>(''); // Состояние для запроса поиска
  ...
  return (
    <header className="Header">
      ...
      <div className="input-wrapper">
        ...
        <input type="search" id="search" name="search" placeholder="Поиск" onChange={handleSearchChange} />
      </div>
        ...
        <div className="search-results">
          {searchResults.map((result) => (
            <div key={result.id} className="search-result">
              <img src={result.header_image_thumbnail_url} alt={result.full_title} />
              <Link to={result.url}>{result.full_title}</Link>
              {/* Вызов функции handleToggleFavorite при клике на кнопку */}
              <button onClick={() => handleToggleFavorite(result)}>Add to Favorites</button>
            </div>
          ))}
        </div>
      )}
